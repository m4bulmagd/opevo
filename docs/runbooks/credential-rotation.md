...
| agent internal token | `AGENT_INTERNAL_API_TOKEN` | Complete — user attested (received 2026-07-13T04:32:42Z) | Complete — user attested (received 2026-07-13T04:32:42Z) | Complete — user attested (received 2026-07-13T04:32:42Z) | User (conversation attestation) | User attestation only; no provider audit ID supplied. | Confirm the new token is accepted and the old token is rejected by an agent-only endpoint. | Passed — user attested | Complete |

...
**Scope:** A desktop visual concept for Opevo's authenticated dashboard
...
The concept must present detailed call analytics using only facts Opevo
...
Opevo is a France-first AI receptionist for professionals and small
...
Use Opevo's approved **Quiet Confidence** direction:
...
- Use the existing Opevo destinations: Overview, Calls, Lea, Billing, and
  Account.
...
Show the illustrative assigned Opevo number **+33 1 87 65 42 10** as
...
The change must be expressed as a count, matching Opevo's current dashboard
...
examples, not a fixed Opevo taxonomy.

### Sentiment

Show a small distribution based on Opevo's existing `sentiment` summary
field:
...
- any control or analytics capability that suggests Opevo currently supports
  it when it does not.
...
- Preserve Opevo's own navigation, terminology, and visual identity.
...
1. it is immediately recognizable as a Opevo call-analytics dashboard;
2. it carries the reference screen's calm, modular information density without
   looking copied;
3. every analytical claim maps to a current Opevo field or a direct aggregate
   of current fields;

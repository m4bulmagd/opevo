  ...
  return (
    <main className="min-h-screen bg-[linear-gradient(180deg,oklch(0.99_0.01_252)_0%,oklch(0.992_0.003_240)_40%,oklch(0.985_0.004_240)_100%)] text-slate-950">
      <div className="relative overflow-hidden">
        ...
        {/*         
        <div className="absolute left-1/2 top-28 h-72 w-72 -translate-x-[145%] rounded-full bg-[oklch(0.92_0.1_252/0.75)] blur-3xl" />
        <div className="absolute right-1/2 top-24 h-80 w-80 translate-x-[130%] rounded-full bg-[oklch(0.84_0.14_257/0.72)] blur-3xl" />
        */}
        ...
        <div className="relative mx-auto flex w-full max-w-6xl flex-col px-6 pt-6 pb-6 sm:px-8 lg:px-10">
          <header className="flex items-center justify-between gap-4">
            <Link
              href="/"
              className="inline-flex items-center gap-3 font-semibold text-slate-900 text-sm uppercase tracking-[0.18em]"
            >
              <span className="flex size-9 items-center justify-center rounded-full border border-slate-200 bg-white/90 shadow-sm">
          ...
          <section className="mx-auto flex max-w-4xl flex-col items-center pt-18 text-center sm:pt-24">
              ...
              {isAuthenticated ? (
                ...
                <>
                  ...
                  <Button
                    ...
                    variant="outline"
                    size="lg"
                    className="rounded-full border-white/80 bg-white/70 px-6 shadow-sm"
                  >
                    <Link href="/sign-in">Log in</Link>
                  </Button>
                </>
      ...
      <section id="features1" className="mx-auto w-full max-w-6xl px-6 pb-8 sm:px-8 lg:px-10">
        ...
        <div className="mt-10 flex flex-wrap items-center justify-center gap-x-8 gap-y-3 text-slate-400 text-sm">
          <span>LiveKit</span>
          <span>Telnyx</span>
          <span>Clerk</span>
          <span>Stripe</span>
      ...
      <section id="features" className="mx-auto w-full max-w-6xl px-6 py-8 sm:px-8 lg:px-10">
        ...
        <div className="mt-12 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {featureCards.map(({ icon: Icon, title, description }) => (
            <article
              key={title}
              className="rounded-[1.75rem] border border-slate-200/80 bg-white px-6 py-6 shadow-[0_20px_60px_-45px_rgba(15,23,42,0.45)]"
            >
        ...
        <div className="mx-auto mt-10 max-w-2xl space-y-3">
          {operationalPoints.map((point) => (
            <div
              key={point}
              className="flex items-center justify-between rounded-full border border-slate-200/80 bg-white px-5 py-3 text-slate-700 text-sm shadow-sm"
            >
        ...
        <div className="mx-auto max-w-6xl">
          ...
          <div className="mt-12 grid gap-5 lg:grid-cols-3">
            {steps.map((step, index) => (
              <article
                key={step.title}
                className="rounded-[1.75rem] border border-white/20 bg-white/12 p-6 shadow-lg backdrop-blur-sm"
              >
      ...
      <section className="mx-auto w-full max-w-6xl px-6 py-18 sm:px-8 lg:px-10">
        ...
        <div className="mt-12 grid gap-5 md:grid-cols-3">
          {secondaryFeatures.map(({ icon: Icon, title, description }) => (
            <article key={title} className="rounded-[1.75rem] border border-slate-200/80 bg-white px-6 py-6 shadow-sm">
              <span className="flex size-11 items-center justify-center rounded-2xl bg-[oklch(0.95_0.03_252)] text-[oklch(0.58_0.17_257)]">
                <Icon className="size-5" />
              </span>
              <h3 className="mt-6 font-medium text-lg text-slate-950">{title}</h3>
      ...
      >
        ...
        <div className="space-y-4">
          {faqs.map(({ question, answer }) => (
            <article
              key={question}
              className="rounded-[1.5rem] border border-slate-200/80 bg-white px-6 py-5 shadow-sm"
            >
      ...
      <footer className="relative overflow-hidden bg-slate-950 px-6 py-14 text-slate-200 sm:px-8 lg:px-10">
        <div className="absolute inset-x-0 bottom-[-11rem] mx-auto h-72 w-[34rem] rounded-full bg-[radial-gradient(circle,rgba(118,140,255,0.95),rgba(118,140,255,0.18)_46%,transparent_72%)] blur-3xl" />
        <div className="relative mx-auto grid w-full max-w-6xl gap-10 lg:grid-cols-[1fr_0.8fr_0.8fr_0.8fr]">
          <div>
